...
Ho allora dovuto usare in un WLS:
\startsh
...
Ho installato la versione a 64 bit. Per lanciarlo in combinazione con {\Context} uso la command line
con questa sequenza:
\startpowsh
...
Ho usato la categoria mingw-w64-ucrt che supporta  Window10/11
...
Siccome in Mysys2 ho diversi ambienti di compilazione installati è risultato che nella bash non avevo più nessun gcc e mingw32-make.
Per questo ho settato nello script iniziale della bash \type{.bash_profile} questo path:
\startpowsh
...
in un commento nel file \type{scite-context-lexer.lua}. L'autore ha mantenuto una dll sulla falsa riga di ScintillaLua
...
scaricando Lpeg da www.inf.puc-rio.br/$\sim$roberto/lpeg. Inizialmente senza risultato in quanto non conosco bene i
...
\stoppowsh
che ingloba  tutte i componenti necessari, vale a dire scintilla, lexilla,  la stdlibc++ di mingw64 e Lpeg.
